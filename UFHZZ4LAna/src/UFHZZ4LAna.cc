...
#include "RecoEgamma/EgammaTools/interface/EffectiveAreas.h"//bing
//Jets
...
private:
    ...
    vector<double> lep_scaleup; vector<double> lep_scaledn; vector<double> lep_smearup; vector<double> lep_smeardn; vector<double> lep_ecalEnergyPostCorr; vector<double> lep_ecalEnergyPreCorr; vector<double> lep_ecalTrkEnergyPostCorr;//bing
    ...
    int nisoleptons;
    double phoRho, muRho, elRho, rhoSUS;//bing
...
//
    ...
    // photon variables
    vector<int> GENpho_id; vector<int> GENpho_status; vector<double> GENpho_pt; vector<double> GENpho_eta; vector<double> GENpho_phi; vector<double> GENpho_energy; vector<int> GENpho_MomId; vector<int> GENpho_MomMomId; vector<double> GENpho_phoIso;//bing
...
//    Mela* GENmela;// FIXME
    ...
    vector<float> GENpho_pt_float, GENpho_eta_float, GENpho_phi_float, GENpho_energy_float, GENpho_phoIso_float;//bing
    ...
    //Input edm
    edm::EDGetTokenT<double> phoRhoSrc_;//bing
    ...
    string PhoMVA_name_161718;//bing
...
UFHZZ4LAna::UFHZZ4LAna(const edm::ParameterSet& iConfig) :
    histContainer_(),
    phoRhoSrc_(consumes<double>(iConfig.getParameter<edm::InputTag>("phoRhoSrc"))),//bing
    chHadIsolationEffAreas((iConfig.getParameter<edm::FileInPath>("phoCIsoeffAreasConfigFile")).fullPath()),//bing
    neuHadIsolationEffAreas((iConfig.getParameter<edm::FileInPath>("phoNIsoeffAreasConfigFile")).fullPath()),//bing
    phoIsolationEffAreas((iConfig.getParameter<edm::FileInPath>("phoPIsoeffAreasConfigFile")).fullPath()),//bing
    ...
    isCode4l(iConfig.getUntrackedParameter<bool>("isCode4l",true))
{
    ...
    if(year==2018)    {EleBDT_name_161718 = "ElectronMVAEstimatorRun2Summer18ULIdIsoValues"; BTagCut=0.4168; heepID_name_161718 = "heepElectronID-HEEPV70";PhoMVA_name_161718="PhotonMVAEstimatorRunIIFall17v2Values";}
    ...
    if(year==2017)    {EleBDT_name_161718 = "ElectronMVAEstimatorRun2Summer17ULIdIsoValues"; BTagCut=0.4506; heepID_name_161718 = "heepElectronID-HEEPV70";PhoMVA_name_161718="PhotonMVAEstimatorRunIIFall17v2Values";}
    ...
    if(year_==2016)    {EleBDT_name_161718 = "ElectronMVAEstimatorRun2Summer16ULIdIsoValues"; BTagCut=0.5847; heepID_name_161718 = "heepElectronID-HEEPV70";PhoMVA_name_161718="PhotonMVAEstimatorRunIIFall17v2Values";}
    if(year_==-2016)    {EleBDT_name_161718 = "ElectronMVAEstimatorRun2Summer16ULIdIsoValues"; BTagCut=0.6001; heepID_name_161718 = "heepElectronID-HEEPV70";PhoMVA_name_161718="PhotonMVAEstimatorRunIIFall17v2Values";}
...
UFHZZ4LAna::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    lep_scaleup.clear(); lep_scaledn.clear(); lep_smearup.clear(); lep_smeardn.clear(); lep_ecalEnergyPostCorr.clear(); lep_ecalEnergyPreCorr.clear(); lep_ecalTrkEnergyPostCorr.clear();//bing
 ...
 */
    ...
    GENpho_pt.clear(); GENpho_eta.clear(); GENpho_phi.clear(); GENpho_energy.clear();//bing
    GENpho_id.clear(); GENpho_status.clear(); GENpho_MomId.clear(); GENpho_MomMomId.clear();//bing
    GENpho_phoIso.clear();//bing
        ...
        if( (recoMuons.size() + recoElectrons.size()) >= (uint)skimLooseLeptons ) {
            ...
            for(unsigned int i = 0; i < lep_ptreco.size(); i++) {
                ...
                if (abs(lep_ptid[i])==11) {
                    ...
                    lep_scaleup.push_back(recoElectrons[lep_ptindex[i]].userFloat("energyScaleUp"));
                    lep_scaledn.push_back(recoElectrons[lep_ptindex[i]].userFloat("energyScaleDown"));
                    lep_smearup.push_back(recoElectrons[lep_ptindex[i]].userFloat("energySigmaUp"));
                    lep_smeardn.push_back(recoElectrons[lep_ptindex[i]].userFloat("energySigmaDown"));
                    lep_ecalEnergyPostCorr.push_back(recoElectrons[lep_ptindex[i]].userFloat("ecalEnergyPostCorr"));
                    lep_ecalTrkEnergyPostCorr.push_back(recoElectrons[lep_ptindex[i]].userFloat("ecalTrkEnergyPostCorr"));
                    lep_ecalEnergyPreCorr.push_back(recoElectrons[lep_ptindex[i]].userFloat("ecalEnergyPreCorr"));
                ...
                if (abs(lep_ptid[i])==13) {
                    ...
                    //bing
                    lep_scaleup.push_back(recoMuons[lep_ptindex[i]].userFloat("scale_unc"));
                    lep_scaledn.push_back(2.0-recoMuons[lep_ptindex[i]].userFloat("scale_unc"));
                    lep_smearup.push_back(recoMuons[lep_ptindex[i]].userFloat("smear_unc"));
                    lep_smeardn.push_back(2.0-recoMuons[lep_ptindex[i]].userFloat("smear_unc"));
                    lep_ecalEnergyPostCorr.push_back(recoMuons[lep_ptindex[i]].pt());
                    lep_ecalTrkEnergyPostCorr.push_back(recoMuons[lep_ptindex[i]].pt());
                    lep_ecalEnergyPreCorr.push_back(recoMuons[lep_ptindex[i]].pt());
            ...
            for(int i = 0; i < (int)recoPhotons.size(); i++) {
                ...
                //auto corr_pho = pho_corr.p4() * pho_corr.userFloat("ecalEnergyPostCorr") / pho_corr.userFloat("ecalEnergyPreCorr");
                //pho_pt.push_back(corr_pho.pt());
                ...
                pho_scaleup.push_back(pho_corr.userFloat("energyScaleUp"));
                pho_scaledn.push_back(pho_corr.userFloat("energyScaleDown"));
                pho_smearup.push_back(pho_corr.userFloat("energySigmaUp"));
                pho_smeardn.push_back(pho_corr.userFloat("energySigmaDown"));
                pho_ecalEnergyErrPostCorr.push_back(pho_corr.userFloat("ecalEnergyErrPostCorr"));
                pho_ecalEnergyPostCorr.push_back(pho_corr.userFloat("ecalEnergyPostCorr"));
                pho_ecalEnergyPreCorr.push_back(pho_corr.userFloat("ecalEnergyPreCorr"));
                
                //pho_ecalTrkEnergyPostCorr.push_back(pho_corr.userFloat("ecalTrkEnergyPostCorr"));
                //cout<<"photon "<<i<<"; pho_pt: "<<recoPhotons[i].pt()<<"; pho_corr_pt: "<<pho_corr.pt()<<"; pho_scaleup: "<<pho_corr.userFloat("energyScaleUp")<<"; pho_scaledn: "<<pho_corr.userFloat("energyScaleDown")<<"; pho_ecalEnergyPostCorr: "<<pho_corr.userFloat("ecalEnergyPostCorr")<<", pho_ecalEnergyPreCorr:"<<pho_corr.userFloat("ecalEnergyPreCorr")<<endl;
...
//next
		    ...
		    // p_GG_SIG_ghg2_1_ghz1_1_ghza1prime2_1E4_JHUGen, ghg2=1,0;ghzgs1_prime2=10000,0
                ...
                //bing
                lep_scaleup_float.assign(lep_scaleup.begin(),lep_scaleup.end());
                lep_scaledn_float.assign(lep_scaledn.begin(),lep_scaledn.end());
                lep_smearup_float.assign(lep_smearup.begin(),lep_smearup.end());
                lep_smeardn_float.assign(lep_smeardn.begin(),lep_smeardn.end());
                lep_ecalEnergyPostCorr_float.assign(lep_ecalEnergyPostCorr.begin(),lep_ecalEnergyPostCorr.end());
                lep_ecalTrkEnergyPostCorr_float.assign(lep_ecalTrkEnergyPostCorr.begin(),lep_ecalTrkEnergyPostCorr.end());
                lep_ecalEnergyPreCorr_float.assign(lep_ecalEnergyPreCorr.begin(),lep_ecalEnergyPreCorr.end());
                ...
                pho_scaleup_float.assign(pho_scaleup.begin(),pho_scaleup.end());
                pho_scaledn_float.assign(pho_scaledn.begin(),pho_scaledn.end());
                pho_smearup_float.assign(pho_smearup.begin(),pho_smearup.end());
                pho_smeardn_float.assign(pho_smeardn.begin(),pho_smeardn.end());
                pho_ecalEnergyErrPostCorr_float.assign(pho_ecalEnergyErrPostCorr.begin(),pho_ecalEnergyErrPostCorr.end());
                pho_ecalEnergyPostCorr_float.assign(pho_ecalEnergyPostCorr.begin(),pho_ecalEnergyPostCorr.end());
                pho_ecalEnergyPreCorr_float.assign(pho_ecalEnergyPreCorr.begin(),pho_ecalEnergyPreCorr.end());
                //bing
...
{     
    ...
    //bing
    tree->Branch("lep_scaleup",&lep_scaleup_float);
    tree->Branch("lep_scaledn",&lep_scaledn_float);
    tree->Branch("lep_smearup",&lep_smearup_float);
    tree->Branch("lep_smeardn",&lep_smeardn_float);
    tree->Branch("lep_ecalEnergyPostCorr",&lep_ecalEnergyPostCorr_float);
    tree->Branch("lep_ecalTrkEnergyPostCorr",&lep_ecalTrkEnergyPostCorr_float);
    tree->Branch("lep_ecalEnergyPreCorr",&lep_ecalEnergyPreCorr_float);
    ...
    tree->Branch("pho_scaleup",&pho_scaleup_float);
    tree->Branch("pho_scaledn",&pho_scaledn_float);
    tree->Branch("pho_smearup",&pho_smearup_float);
    tree->Branch("pho_smeardn",&pho_smeardn_float);
    tree->Branch("pho_ecalEnergyPostCorr",&pho_ecalEnergyPostCorr_float);
    tree->Branch("pho_ecalEnergyErrPostCorr",&pho_ecalEnergyErrPostCorr_float);
    tree->Branch("pho_ecalEnergyPreCorr",&pho_ecalEnergyPreCorr_float);
